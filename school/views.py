...
def afterlogin_view(request):
    ...
    if str(is_admin(request.user))=="True":
        return redirect('/admin-dashboard')
    elif str(request.user.groups.filter(name='TEACHER').exists())=="True":
        accountapproval=models.TeacherExtra.objects.all().filter(user_id=request.user.id,status=True)
        print(str(accountapproval))
        if accountapproval:
            return redirect('teacher-dashboard')
        else:
            return redirect('/teacherwaitapproval')
    elif is_student(request.user)==True:
        accountapproval=models.StudentExtra.objects.all().filter(user_id=request.user.id,status=True)
        if accountapproval:
            return redirect('student-dashboard')
        else:
            return redirect('/studentwaitapproval')
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        #sub = forms.ContactusForm(request.POST)
            ...
            Message.objects.get_or_create(name=name,email=email,message=message)
...
def teacherwait_approval(request):
    return render(request, 'school/teacher_wait_for_approval.html', {})


def admin_message_view(request):
    messages=models.Message.objects.all()
    return render(request, 'school/admin_message.html', {'messages':messages})



def delete_message_from_database(request, pk):
